...
def test_get_book_by_id_success(
    client: TestClient, book_id: str, expected_status: int, expected_title: str
):
...
def test_get_book_by_id_fail(client: TestClient, book_id: str, expected_status: int):
    response = client.get(f"/books/{book_id}")
    assert response.status_code == expected_status
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            None,
            200,
            ["KY0BDObXftUC", "H8ON-dTgQQYC", "Wy0svf_7NzsC", "ouD2ugAACAAJ"],
        ),
        ("2001", ["J.R.R. Tolkien"], None, None, None, 200, ["tFWlPwAACAAJ"]),
        ("2001", ["J.R.R. Tolkien"], None, None, None, 200, ["tFWlPwAACAAJ"]),
        (
            "2001",
            ["Chuck Dixon", "John Ronald Reuel Tolkien"],
            None,
            None,
            None,
            200,
        ...
        (
            ...
            3,
            200,
            ["N_0VhzQKIIAC", "U4nM6f26XjgC", "8ef3-s6fixIC"],
        ),
...
def test_get_books_endpoint(
    ...
    response = client.get("/books", params=query)
